...
// SCENE SHADER (Double-sided lighting fix)
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // FIX: Use abs(dot) to light both sides of the wall equally.
    // This ensures that even if winding is backwards (backface), the wall is lit.
...
fn fs_main(@builtin(position) frag_coord: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let total_chars = 8 + num_digits + 1; 
    ...
    if (screen_pos.y >= text_start_y && screen_pos.y < text_start_y + char_h) {
        ...
        if (rel_x >= 0.0 && rel_x < total_w) {
            ...
            if (in_x < char_w) {
                ...
                var c = 32;
                if (slot == 0) { c = 76; } else if (slot == 1) { c = 111; } else if (slot == 2) { c = 97; } else if (slot == 3) { c = 100; } else if (slot == 4) { c = 105; } else if (slot == 5) { c = 110; } else if (slot == 6) { c = 103; } else if (slot == 7) { c = 32; } else if (slot < 8 + num_digits) {
                    let d_idx = slot - 8;
                    var d = 0;
